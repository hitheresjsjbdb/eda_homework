...
class EngineIMAP():
    ...
    def rewrite(self, priority_size = 10, cut_size = 4, level_preserve = False, zero_gain = False, verbose = False):
        ...
        Args:
            priority_size (int, optional): The max store size of the cut for a node. Range of [6,12]. Defaults to 10.
    ...
    def history(self, clear = False, size = False, add = False, replace = -1, backup = -1, erase = -1):
        ...
        Args:
            ...
            erase (int, optional): Erase the history AIG with the given index. Defaults to -1.
        """
        imap.history(clear = clear, size = size, add = add, replace = replace, backup = backup, erase = erase)
