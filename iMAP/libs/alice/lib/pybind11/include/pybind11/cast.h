...
PYBIND11_NOINLINE inline std::string error_string() {
    ...
    if (scope.trace) {
        ...
        while (frame) {
            ...
            PyObject *co_filename = f_code ? PyObject_GetAttrString((PyObject *) f_code, "co_filename")
                                           : nullptr;
            PyObject *co_name = f_code ? PyObject_GetAttrString((PyObject *) f_code, "co_name")
                                       : nullptr;
            PyFrameObject *next = PyFrame_GetBack(frame);

            if (co_filename && co_name) {
                errorString +=
                    "  " + handle(co_filename).cast<std::string>() +
                    "(" + std::to_string(lineno) + "): " +
                    handle(co_name).cast<std::string>() + "\n";
            } else {
                PyErr_Clear();
            }
