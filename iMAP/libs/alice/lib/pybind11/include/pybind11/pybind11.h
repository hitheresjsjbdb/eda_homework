...
inline function get_type_overload(const void *this_ptr, const detail::type_info *this_type, const char *name)  {
    ...
    if (frame) {
        ...
        PyObject *co_name = f_code ? PyObject_GetAttrString((PyObject *) f_code, "co_name") : nullptr;
        PyObject *co_argcount = f_code ? PyObject_GetAttrString((PyObject *) f_code, "co_argcount") : nullptr;
        bool is_same_name = co_name && (std::string) str(co_name) == name;
        long argcount = co_argcount ? PyLong_AsLong(co_argcount) : 0;

        if (is_same_name && !PyErr_Occurred() && argcount > 0) {
            PyObject *locals = PyFrame_GetLocals(frame);
            PyObject *co_varnames = f_code ? PyCode_GetVarnames(f_code) : nullptr;
            PyObject *self_arg = co_varnames ? PyTuple_GetItem(co_varnames, 0) : nullptr;
            PyObject *self_caller = (locals && self_arg) ? PyObject_GetItem(locals, self_arg) : nullptr;
